# src/services/leagues_service.py
...
def add_team_service(club_name, league_name, season):
    """
    Делегира изцяло към leagues_repo.add_team_to_league,
    която сама прави всички проверки.
    БЪГЪТ: по-рано се подаваше league_id вместо league_name + season.
    """
    club_name   = normalize_text(club_name)
    league_name = normalize_text(league_name)
    season      = normalize_text(season)

    # add_team_to_league(club_name, league_name, season) – вижда се в leagues_repo.py
...
def generate_schedule_service(league_name, season):
    ...
    if len(teams) < 4:
        return "❌ Минимум 4 отбора за генериране на програма."

    # Проверка дали вече е генерирана
    from repositories.leagues_repo import league_has_matches
    if league_has_matches(league_id):
        return "⚠️ Програмата вече е генерирана за тази лига."
    ...
    matches_per_round = len(schedule) // rounds if rounds else 0
    return f"✅ Генерирани {rounds} кръга с по {matches_per_round} мача."
